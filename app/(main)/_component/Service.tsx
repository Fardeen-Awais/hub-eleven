...
const Service = () => {
  ...
  return (
    <div className='flex flex-col justify-center items-center min-h-screen gap-5 p-10'>
      ...
      <div className='flex flex-col items-center gap-2 mx-auto max-w-3xl p-5'>
        <div className='grid grid-cols-1 md:grid-cols-2 gap-2 '>
          {cardData.map((card, index) => (
            <ServiceCards key={index} title={card.title} description={card.description} image={card.image} />
          ))}
        </div>
        <Card className='flex flex-col overflow-hidden justify-center items-center py-2 sm:max-h-52  '>
          <CardContent className='flex flex-col sm:flex-row justify-center items-center gap-2 p-5'>
            <div className='flex flex-col justify-center gap-2 '>
              <CardTitle className='text-3xl font-semibold'>
                Digital Marketing
              </CardTitle>
              <CardDescription className='max-w-md text-sm'>We provide a range of digital marketing solutions to help your business succeed. Help you to manage your business compaigns</CardDescription></div>
