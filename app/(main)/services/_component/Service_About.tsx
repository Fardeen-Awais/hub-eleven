        ...
        {data.map((item:any, index:number) => (
            <div className='flex flex-col sm:flex-row items-center gap-5 sm:gap-40' key={index}>
                ...
                <div className='flex justify-between'>
                    <div className='hidden sm:flex justify-between items-center mx-auto w-full gap-10'>
                        <p className='text-xl '>{item.description}</p>
                        <Image className='w-96 h-full relative ' src={item.image} width={500} height={500} alt='Digital marketing related picture' />
                    </div>
                    ...
                    <div className='flex flex-col sm:flex-row items-center justify-center gap-10'>
                        <Image className='w-60 h-60 lg:w-96 lg:h-full relative ' src={item.image} width={500} height={500} alt='Digital marketing related picture' />
                        <p className='text-xl max-w-full mx-5 '>{item.description}</p>
                    </div>
                )}
            </div>
        ))}
